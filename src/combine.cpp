...
static bool chunk_ends_type(chunk_t *start)
{
   ...
   for (/* nada */; pc != NULL; pc = chunk_get_prev_ncnl(pc))
   {
      ...
      if (chunk_is_semicolon(pc) ||
          ...
          (pc->type == CT_FPAREN_OPEN && pc->parent_type == CT_FUNC_CALL && cpd.lang_flags & LANG_OC) ||
          chunk_is_forin(pc) ||
...
void do_symbol_check(chunk_t *prev, chunk_t *pc, chunk_t *next)
{
   ...
   if ((pc->type == CT_MINUS) ||
       (pc->type == CT_PLUS))
   {
      if ((prev->type == CT_POS) || (prev->type == CT_NEG) || (cpd.lang_flags & LANG_OC && prev->type == CT_OC_AT ))
      {
